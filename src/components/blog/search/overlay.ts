...
class SearchDialog {
  ...
  constructor(
    ...
    this.template = dialog.querySelector(
      ".result-template",
    ) as HTMLTemplateElement;
    this.clear_button = dialog.querySelector(
      ".clear-query",
    ) as HTMLButtonElement;
    this.group_template = dialog.querySelector(
      ".result-group-header-template",
    ) as HTMLTemplateElement;
    this.default_content = dialog.querySelector(
      ".default-content",
    ) as HTMLElement;
      ...
      if (!timeout) {
        timeout = window.setTimeout(() => {
          sessionStorage.setItem(
            "scroll-top",
            this.results.scrollTop.toString(),
          );
          timeout = 0;
        }, 250);
      }
  ...
  public SetContent(
    ...
    data: Record<
      string,
      string | { text?: string; attributes: Record<string, string> }
    >,
  ) {
  ...
  public PopulateResults(list: CollectionSearchResult[]) {
    ...
    if (!list.length) {
      ...
      for (const [collection, list] of Object.entries(groups)) {
        const header = this.group_template.content.cloneNode(
          true,
        ) as HTMLElement;
        this.SetContent(header, {
          "result-group-header": this.options.group_header(collection),
        });
        ...
        this.results.append(
          ...list.map((result) => {
            ...
            const description = this.options.format_description
              ? this.options.format_description(result)
              : result.frontmatter.description || "No description";

            const title = this.options.format_title
              ? this.options.format_title(result)
              : result.frontmatter.title || "No title";
  ...
  public Init() {
    ...
    if (text) {
      ...
      const element = this.dialog.querySelector(
        `[data-index="${this.selected_index}"]`,
      );
